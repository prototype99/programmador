...
pkg_setup_archlist() {
	...
	use android-arch_mips && android_archlist+=("mips") # fails at linking qt3d/tools/qgltf
	use android-arch_mips && ewarn "Do not use mips! Qt will not be compiled (tested with NDK_13)"
	use android-arch_mips64 && android_archlist+=("mips64") # fails at compiling apk
	use android-arch_mips && ewarn "Do not use mips64! Apk will not be compiled (tested with NDK_13)"
...
src_configure_arch() {
	...
	# Choosing toolkit which is at least capable to compile for selected arch at all
	case "$android_arch" in
...
src_install_arch() {
	...
	case "$android_arch" in
		...
		mips64)
			...
			stripFolder=mips64el-linux-android
			;;
		x86)
			toolchain=x86-4.9
